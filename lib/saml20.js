...
exports.create = function(options, callback) {
  ...
  if (options.attributes) {
    ...
    Object.keys(options.attributes).forEach(function(prop) {
      ...
      attributeElement.setAttribute('NameFormat', 
        options.attributes[prop].key && options.attributes[prop].nameFormat ? 
            options.attributes[prop].nameFormat:
            "urn:oasis:names:tc:SAML:2.0:attrname-format:basic"
      );
      var values = options.attributes[prop].values instanceof Array ? options.attributes[prop].values : [options.attributes[prop].values];
      values.forEach(function (value) {
        var valueElement = doc.createElementNS(NAMESPACE, 'saml:AttributeValue');
        valueElement.setAttribute('xsi:type', value.type ? value.type : 'xs:anyType');
